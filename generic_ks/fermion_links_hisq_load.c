...
/* (Most recently the file name was fermion_links_hisq_load_milc.c) */
/* (Prior to that fermion_links_hisq.c AND fermion_links_helpers_hisq.c) */
/* MILC Version 7 */
/* Top-level link fattening for both CPU (QOP and MILC) and GPU
   Thesse routines are called from fermion_links_hisq_milc.c
   They call fermion_links_hisq_load_*.c and fermion_links_fn_milc.c */

/* Entry points

   destroy_hisq_auxiliary_t
   create_hisq_links_milc
   destroy_hisq_links_milc
...
/*-------------------------------------------------------------------*/
/* Special memory allocations for field with four su3_matrices per site */
/*-------------------------------------------------------------------*/
...
#if defined(USE_REUNIT_GPU)

/* DON'T USE: The needed GPU support is not yet available */
